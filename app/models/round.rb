class Round < ActiveRecord::Base
  ...
  #   def self.leaders
  #     ids = Round.select('MAX(total_score) as max_score, user_id, id').having('MAX(rounds.id) > 0').group(:user_id).order('max_score DESC').limit(10).map(&:id)
  #     Round.where('id in (?)', ids)
  #   end
